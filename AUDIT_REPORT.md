...
Systematic audit of havn's current state across specs, implementation, tests,
code quality, infrastructure, and documentation. Each section corresponds to a
...
havn is architecturally sound and well-engineered for its stage. The spec suite
is comprehensive and internally consistent, domain code faithfully implements
spec requirements, and test quality is high. The project is in a "skeleton
complete, wiring pending" state — the critical path to a working CLI is short.

**Key metrics:**

| Metric | Value |
|--------|-------|
| Spec requirements audited | 99 |
| Requirements MET | 87 (88%) |
| Requirements PARTIAL | 8 (8%) — domain code ready, CLI wiring missing |
| Requirements MISSING | 4 (4%) — blocked on Dockerfile/base image spec |
| Test coverage (overall) | 78.5% |
| Packages at 80%+ coverage | 8 of 10 |
| External dependencies | 4 (all well-established) |
| Spec contradictions found | 3 (all low severity) |
| Underspecified areas | 6 |

**What's working well:**

- Architecture strictly follows specs: dependency isolation, consumer-defined
  interfaces, error handling patterns, stream separation, type system
- Domain packages (`container`, `dolt`, `doctor`, `volume`, `mount`, `name`)
  are fully implemented and tested
- All 12 stub CLI commands have complete backing domain code
- Test patterns are exemplary: black-box by default, fakes over mocks,
  table-driven, testify, proper naming conventions
- Build tooling (Makefile, lefthook, golangci-lint) matches quality-gates spec

**What needs attention (priority order):**

1. **CLI wiring** — 12 commands return `ErrNotImplemented` despite domain code
   being ready. This is the highest-value next step.
2. **CI/CD pipeline** — quality gates run only locally; no enforcement on push.
3. **Compile-time interface assertions** — missing in `internal/docker/`;
   the only spec-required pattern not yet implemented.
4. **Integration tests** — zero exist despite infrastructure being defined.
   Docker wrapper success paths are unverified.
5. **Documentation** — README is empty, no user-facing docs, no CONTRIBUTING.md.
6. **Dockerfile / base image spec** — blocks `havn build` and full e2e startup.

---

## Table of Contents

1. [Spec Consistency](#spec-consistency)
2. [Implementation Gap Analysis](#implementation-gap-analysis)
3. [Code Quality and Architecture](#code-quality-and-architecture)
4. [Test Coverage and Quality](#test-coverage-and-quality)
5. [Infrastructure](#infrastructure)
6. [Documentation and Onboarding](#documentation-and-onboarding)
...
_Consolidated from all audit sections. Organized by priority._

### Critical (blocks user-facing functionality)

1. **Wire CLI commands to domain code** — All 12 stub commands have complete
   domain implementations. The gap is exclusively CLI wiring in `Execute()`:
   initialize missing `Deps` fields (`DoctorBackend`, `VolumeManager`,
   `DoltManager`, container backends) and write `RunE` functions.
   _(Implementation Gap Analysis)_

2. **Create CI/CD pipeline** — Quality gates run only locally via lefthook.
   A GitHub Actions workflow running `make check` would enforce gates on every
   push. Integration tests should be gated on Docker availability.
   _(Infrastructure)_

3. **Write base image implementation spec** — Unblocks Dockerfile creation,
   `havn build`, and full end-to-end startup. Must cover Dockerfile,
   entrypoint, init script, and UID/GID mapping.
   _(Spec Consistency, Implementation Gap Analysis)_

### High (correctness and confidence)

4. **Add compile-time interface assertions** in `internal/docker/` — The only
   spec-required pattern not yet implemented. Missing for ~9 consumer-defined
   interfaces. Catches interface drift at declaration site instead of call site.
   _(Code Quality)_

5. **Create integration tests for docker wrapper** — Success paths
   (`ContainerList`, `ContainerInspect`, `NetworkInspect`, `VolumeList`)
   are untested. Use `//go:build integration` tag. Infrastructure is already
   defined in Makefile.
   _(Test Coverage)_

6. **Add `ContainerAttach` integration test** with PTY simulation — 14.8%
   coverage; interactive I/O flow is entirely unverified.
   _(Test Coverage)_

### Medium (spec hygiene and consistency)

7. **Fix shared-dolt-server.md section 4** — Add missing
   `BEADS_DOLT_SERVER_DATABASE` env var to lifecycle sequence.
   _(Spec Consistency)_

8. **Fix shared-dolt-server.md section 8** — Change "interactive confirmation"
   to "requires `--yes` flag" for `havn dolt drop`.
   _(Spec Consistency)_

9. **Clarify `dolt.enabled` default** in shared-dolt-server.md Configuration
   Reference — Show `enabled = false` or note the example shows opt-in.
   _(Spec Consistency)_

10. **Add `sloglint` to code-standards.md section 6** — The linter is in
    `.golangci.yml` enforcing section 5's slog conventions but not documented
    in section 6.
    _(Spec Consistency)_

11. **Specify underspecified config fields** — `ports`, `[environment]`,
    `memory_swap`, and `--port` / SSH port mapping appear in spec examples
    but lack behavioral documentation.
    _(Spec Consistency)_

### Low (documentation and polish)

12. **Write README.md** — Add project description, prerequisites, installation,
    basic usage, link to `specs/`.
    _(Documentation)_

13. **Write CONTRIBUTING.md** — Dev setup, make targets, test commands, PR
    workflow.
    _(Documentation)_

14. **Fix stale `docs/QUICKSTART.md` reference** in AGENTS.md — File does
    not exist.
    _(Documentation)_

15. **Inject logger into domain packages** — Infrastructure is ready
    (`Deps.Logger` exists); constructors need `*slog.Logger` parameter
    when logging calls are added.
    _(Code Quality)_

16. **Start CHANGELOG.md** — Before first public release.
    _(Documentation)_
